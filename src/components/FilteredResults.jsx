...
export default function FilteredResults({ filters }) {
  ...
  const results = allServices.filter(
    (s) =>
      ...
      (!filters.issue || s.name.toLowerCase().includes(filters.issue.toLowerCase())) &&
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      {results.length === 0 ? (
        <p className="text-gray-600">❌ No services found. Try different filters.</p>
      ) : (
        <div className="grid md:grid-cols-2 gap-6">
          {results.map((item) => (
            <div
              key={item.id}
              className="border rounded-lg shadow-sm p-6 bg-white flex flex-col justify-between"
            >
              <div>
                <h3 className="text-lg font-semibold text-[#1A2A49]">{item.name}</h3>
              ...
              <div className="mt-4">
                <button
                  onClick={() =>
                    navigate(`/request/${item.name.replace(/\s+/g, "-").toLowerCase()}`)
                  }
                  className="w-full py-2 rounded-lg bg-[#1A2A49] text-white hover:bg-[#223a61]"
                >
                  Book Now
                </button>
